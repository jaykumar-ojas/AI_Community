...
const Message = ({ 
  ...
  return (
    <div className={`mb-4 ${depth > 0 ? `ml-${Math.min(depth * 4, 12)}` : ''} relative`}>
      {/* Vertical connecting line for nested replies with animation - with lower z-index */}
      {depth > 0 && (
        <div 
          className={`absolute left-0 top-0 bottom-0 border-l-2 ${threadColor} transition-all duration-300 hover:border-l-3`} 
          style={{ 
            height: '100%', 
            left: `${(depth-1) * 16}px`,
            boxShadow: '0 0 5px rgba(0, 0, 0, 0.1)',
            zIndex: 1 // Lower z-index so it appears behind messages
          }}
        ></div>
      )}
      
      {/* Horizontal connecting line for nested replies with animation - with lower z-index */}
      {depth > 0 && (
        <div 
      ...
      {parentUserName && depth > 0 && (
        <div className="text-xs text-gray-500 mb-1 ml-2 font-medium relative z-10">
          Replying to <span className="font-semibold text-blue-600 hover:underline cursor-pointer">{parentUserName}</span>
        </div>
      )}
      
      <div className={`rounded-lg shadow-sm p-3 ${isAuthor ? 'bg-blue-50' : 'bg-white'} 
        ${depth > 0 ? `border-l-2 ${threadColor}` : ''} 
        transition-all duration-200 hover:shadow-md relative z-10`}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center">
            <span className="font-medium text-blue-600 text-sm hover:underline cursor-pointer">{userName}</span>
        ...
        {mediaAttachments && Array.isArray(mediaAttachments) && mediaAttachments.length > 0 && (
          <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
            {mediaAttachments.map((attachment, index) => (
              <div key={index} className="relative overflow-hidden rounded-md shadow-sm border border-gray-200 bg-gray-50 transition-transform duration-300 hover:scale-105" style={{ maxWidth: '120px', height: 'auto' }}>
        ...
        <div className="flex items-center mt-2 text-xs">
          <button
            onClick={onLike}
            className={`flex items-center mr-3 ${isLiked ? 'text-blue-600' : 'text-gray-500'} hover:text-blue-600 transition-colors duration-200`}
          >
          ...
          <button
            onClick={onDislike}
            className={`flex items-center mr-3 ${isDisliked ? 'text-red-600' : 'text-gray-500'} hover:text-red-600 transition-colors duration-200`}
          >
      ...
      {/* View more button for deep nesting with improved styling */}
      {showViewMoreButton && (
        <button 
          onClick={onViewMore}
          className={`mt-2 text-sm text-blue-600 hover:text-blue-800 flex items-center transition-colors duration-200 
                     px-3 py-1 rounded-md hover:bg-blue-50 ${threadColor.replace('border', 'text')} relative z-10`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
