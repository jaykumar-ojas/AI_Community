...
const UserHeaderEdit = () => {
    ...
    const { id } = useParams();
    const isCurrentUser = loginData?.validuserone?._id === id || false;
...
>
  ...
  {isOwnProfile && (
    <div className="absolute inset-0 z-10 hidden group-hover:flex items-center justify-center bg-black bg-opacity-50 cursor-pointer transition-all duration-300">
      <span className="bg-white text-black px-4 py-1 rounded-full text-sm shadow">Browse Background</span>
    </div>
  )}
  ...
  {/* Profile Picture */}
  <div className="absolute bottom-0 transform left-1/2 translate-y-1/2 sm:left-[8%] sm:translate-x-0 sm:translate-y-2/3 lg:translate-y-3/4">
    <div className="relative group w-28 h-28 md:w-36 md:h-36 sm:w-32 sm:h-32 lg:w-48 lg:h-48 rounded-[30px] overflow-hidden border-4 border-white shadow-lg bg-cover bg-center"
      style={{ backgroundImage: `url(${profileUser?.profilePictureUrl})` }}
    >
      {/* Profile hover overlay */}
      {isOwnProfile && (
        <div className="absolute inset-0 z-10 hidden group-hover:flex items-center justify-center bg-black bg-opacity-50 cursor-pointer transition-all duration-300">
          <span className="bg-white text-black px-3 py-1 rounded-full text-sm shadow">Browse Profile</span>
        </div>
      )}
                                ...
                                {!isOwnProfile && loginData?.validuserone && (
                                    <div className="flex justify-center sm:justify-start mt-3">
                                        <button
                                            ...
                                            className={`px-6 py-2 rounded-full font-medium text-sm transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${isSubscribed
