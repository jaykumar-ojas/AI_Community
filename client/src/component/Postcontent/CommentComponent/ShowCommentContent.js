...
const ShowCommentContent = ({reply,showViewMore,onViewMore,hasChildren,
  show,
  showReply,
  setShowReply}) => {
    ...
    const getTrimmedContent = (text) => {
      ...
      const handleDeleteReply = async () => {
        ...
        try {
          ...
          const response = await axios.delete(`http://localhost:8099/comments/${reply?._id}`, {
  ...
  const handleReplyLike = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:8099/comments/${reply?._id}/like`, {}, {
        headers: getAuthHeaders()
      });
  ...
  const handleReplyDislike = async (replyId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8099/comments/${reply?._id}/dislike`, {}, {
        headers: getAuthHeaders()
      });
        if (response.status === 200) {
