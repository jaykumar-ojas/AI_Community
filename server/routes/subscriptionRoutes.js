...
router.post('/subscribe/:userId', authenticate, async (req, res) => {
    ...
    try {
        ...
        let userToSubscribe = await userdb.findById(subscribedToId) || await googledb.findById(subscribedToId);
...
router.get('/subscriptions', authenticate, async (req, res) => {
    try {
        ...
        const subscribedToIds = subscription.subscribedTo;
        
        // Fetch both regular and Google users
        const regularUsers = await userdb.find({ _id: { $in: subscribedToIds } })
            .select('userName profilePicture profilePictureUrl');
            
        const googleUsers = await googledb.find({ _id: { $in: subscribedToIds } })
            .select('userName profilePicture profilePictureUrl');
